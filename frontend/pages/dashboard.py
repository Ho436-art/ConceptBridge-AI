...
def show():
    ...
    # Calculate metrics based strictly on authentic user database records
    ...
    study_seconds = sum([int(h.get("time_spent_seconds", 0) or 0) for h in db_history]) if db_history else 0
    study_mins = study_seconds // 60
    ...
    if db_mastery:
        ...
        st.info("🌱 **No topics learned yet.** Head over to the **Learning Hub** to ask your first question and start building your mastery score!")
    ...
    with col_breakdown:
        ...
        if not db_mastery:
            st.caption("Mastery health data will appear here as you answer understanding check questions in the Learning Hub.")

    with col_rec:
        st.markdown("### 🎯 Recommended Next Steps")
        st.caption("AI-curated learning pathway based on your mastery profile:")
        
        if db_mastery or db_history:
            current_topic = db_history[0]["title"] if db_history else (db_mastery[0]["title"] if db_mastery else "Recursion")
